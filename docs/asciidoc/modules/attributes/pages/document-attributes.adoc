= Getting Started with Document Attributes
...
** text formatting (if wrapped in a xref:subs:pass-macro.adoc#pass-subs[pass macro])
** inline macros (if wrapped in a xref:subs:pass-macro.adoc#pass-subs[pass macro])
