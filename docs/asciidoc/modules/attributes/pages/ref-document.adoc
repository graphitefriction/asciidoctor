...
|{y}
|{y}
|Controls how xref:catch-errors.adoc[missing references] are handled.
...
|{y}
|{y}
|Controls how xref:catch-errors.adoc[undefined attributes] are handled.
...
|{y}
|
|Label for program name section in the manpage.
...
|{y}
|
|Controls automatic equation numbering on LaTeX blocks in HTML output (MathJax). See https://docs.mathjax.org/en/v2.5-latest/tex.html#automatic-equation-numbering[equation numbering in MathJax].
...
|{y}
|{n}
|Source (e.g., application and version) the manpage describes.
//<<man-pages>>
