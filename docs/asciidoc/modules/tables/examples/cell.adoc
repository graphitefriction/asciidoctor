...
|This cell *is not* prefixed with an `a`, so the processor doesn't interpret the listing block delimiters or the `source` style.
...
l|This cell is treated like a literal block (l).

v|This cell contains a xref:blocks:verse.adoc[verse block] (`v`)
