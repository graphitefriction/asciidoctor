...
Taking into account any xref:span.adoc[spans], which are applied via a <<specifiers,cell specifier>>, each row consists of the same number of cells.
...
* xref:span.adoc[span]
* xref:copy-cell-content.adoc[duplication]
...
|===
----
<1> Separate rows by one or more blank lines.
...
<2> The first row is entered on the line directly after the opening table delimiter (`|===`) and followed by a blank line.
