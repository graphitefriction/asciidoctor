...
Using the `tags` attribute (or `tag` attribute for the singular case), you can select content that falls between regions marked with user-defined tags.
...
`**`::
Select all the lines in the document except for lines containing a tag directive.
