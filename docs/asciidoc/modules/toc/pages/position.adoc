...
By default, the Table of Contents (TOC) is inserted directly below the document header (document title, author, and revision lines) when the `toc` attribute is set.
This location can be changed by assigning a built-in position value to `toc`.

[#left-or-right-column-layout]
== Display TOC as a side column
...
[#manual-placement]
== Display TOC in the document body
...
CAUTION: When using the preamble placement, the TOC will not appear if your document does not have a xref:blocks:preamble-and-lead.adoc[preamble].
...
[source]
----
include::example$toc-macro.adoc[]
----
