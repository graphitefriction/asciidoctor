...
// tag::group[]
`none`:: Substitution group that disables all substitution steps.

`normal`:: Substitution group that performs all substitution steps but doesn't process callouts.
...
The output of `replacements` may depend on whether the `specialcharacters` substitution step was previously applied.
...
== Setting the subs attribute on a block
...
[#incremental]
== Adding and removing substitution steps from a default substitution group
...
You can add or remove a substitution step from the default substitution group using the plus (`+`) and minus (`-`) modifiers.
...
You can also specify whether the substitution step is added to the end of the substitution group.
...
Another way to ensure consistency and keep your documents clean and simple is to use the xref:asciidoctor:api:tree-processor.adoc[Tree Processor extension].
====
