...
<<table-subs-groups>> shows the substitution steps that are executed in each group.
...
No substitution steps are applied to three of the elements in the pass substitution group (`pass`).
These elements include the xref:pass-block.adoc[passthrough block], xref:pass-macro.adoc#inline-pass[inline pass macro], and xref:pass-macro.adoc#triple-plus[triple plus macro].

The xref:pass-macro.adoc#def-plus[inline single plus and double plus macros] also belong to the pass group.
Only the special characters substitution step is applied to these elements.

[#none-group]
=== None substitution group

The none substitution group (`none`) is applied to comment blocks.
No substitution steps are applied to comments.
