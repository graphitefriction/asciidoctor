...
Asciidoctor is a fast, open source text processor for parsing AsciiDoc into a document model and/or converting it to output formats such as HTML 5, DocBook, PDF, and EPUB 3.
To simplify installation, Asciidoctor is packaged and distributed as a RubyGem (aka gem) to {url-rubygem}[RubyGems.org^], which can be installed on all major operating systems using Ruby packaging tools.
